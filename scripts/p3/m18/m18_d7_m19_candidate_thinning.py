...
def score_candidate(r: dict[str, Any]) -> tuple[int, list[str]]:
    ...
    if duration_windows > 0:
        add = min(duration_windows * 6, 50)
        score += add
        reasons.append(f"duration_windows_bonus_{add}")
    ...
    # 有明确 prefix/asn/tal 才适合 M19 mapping。
    if r.get("prefix") and r.get("asn"):
        score += 10
        reasons.append("prefix_asn_available")
    else:
        score -= 20
        reasons.append("prefix_asn_missing_penalty")

    if r.get("tal"):
        score += 5
        reasons.append("tal_available")
...
def main() -> None:
    ...
    ap.add_argument("--per-tal-cap", type=int, default=300)
    ...
    for r in iter_jsonl(in_path):
        ...
        score, score_reasons = score_candidate(r)
        r2 = dict(r)
        r2["m18_d7_score"] = score
        r2["m18_d7_score_reasons"] = score_reasons
        r2["m18_d7_selection_scope"] = "candidate_level_not_causal_attribution"

        scored.append(r2)

        by_tal[str(r.get("tal") or "unknown")] += 1
        by_class[str(r.get("transient_or_persistent") or "unknown")] += 1
        by_asn[str(r.get("asn") or "unknown")] += 1

    scored.sort(
        key=lambda x: (
            safe_int(x.get("m18_d7_score")),
    ...
    # 第一轮：带 TAL / ASN cap 的均衡筛选。
    for r in scored:
        if len(selected) >= args.top_n:
            break

        key = r.get("vrp_key")
        tal = str(r.get("tal") or "unknown")
        asn = str(r.get("asn") or "unknown")

        if key in selected_keys:
            continue
        if tal_used[tal] >= args.per_tal_cap:
            continue
    ...
    topn_path = out_dir / f"M18_to_M19_seed_candidates_top{args.top_n}.jsonl"
    top_small_path = out_dir / f"M18_to_M19_seed_candidates_top{args.top_small_n}.jsonl"
    summary_path = out_dir / "M18_to_M19_seed_candidates_summary.json"
    check_path = out_dir / "M18_D7_M19_CANDIDATE_THINNING_CHECK.txt"
    ...
    summary = {
        "schema": "s3.m18.d7.m19_candidate_thinning.summary.v1",
        ...
        "per_tal_cap": args.per_tal_cap,
        "per_asn_cap": args.per_asn_cap,
        "input_by_tal_top20": by_tal.most_common(20),
        "input_by_class": dict(by_class),
        "input_by_asn_top20": by_asn.most_common(20),
        ...
        "score_range": {
            "max": top_n[0].get("m18_d7_score") if top_n else None,
            "min": top_n[-1].get("m18_d7_score") if top_n else None,
        },
        "outputs": {
    ...
    lines = [
        "M18_D7_M19_CANDIDATE_THINNING=PASS",
        ...
        f"selected_by_class = {summary['selected_by_class']}",
    ...
    state = Path("data/p3_collector/m18_deep_analysis/state/current_m18_d7_run.env")
    state.write_text(
        "\n".join([
            f'export M18_D7_OUT_DIR="{out_dir}"',
            f'export M18_D7_CHECK="{check_path}"',
            f'export M18_D7_SUMMARY="{summary_path}"',
            f'export M18_D7_TOPN="{topn_path}"',
            f'export M18_D7_TOP_SMALL="{top_small_path}"',
            "",
        ]),
        encoding="utf-8",
    )
