...
def main() -> None:
    ...
    parse_records_r2_path = indexes / "manifest_filelist_parse_records_r2.jsonl"
    filelist_entries_r2_path = indexes / "manifest_filelist_entries_r2.jsonl"
    reverse_index_r2_path = indexes / "roa_uri_to_manifest_filelist_index_r2.jsonl"
    summary_r2_path = outputs / "M22B_manifest_filelist_parse_summary_r2.json"
    check_r2_path = checks / "M22B_manifest_filelist_parse_check_r2.txt"

    input_records = read_jsonl(input_records_path)

    parse_records_r2: list[dict[str, Any]] = []
    filelist_entries_r2: list[dict[str, Any]] = []
    reverse_index_r2: list[dict[str, Any]] = []
    ...
    filelist_entry_count = len(filelist_entries_r2)
    ...
    summary = {
        ...
        "roa_uri_to_manifest_filelist_index_count": len(reverse_index_r2),
        ...
        "important_boundary": [
            "R2 parses Routinator cache wrapper by extracting embedded CMS DER.",
        ...
        "outputs": {
            "manifest_filelist_parse_records_r2": str(parse_records_r2_path),
            "manifest_filelist_entries_r2": str(filelist_entries_r2_path),
            "roa_uri_to_manifest_filelist_index_r2": str(reverse_index_r2_path),
            "summary_r2": str(summary_r2_path),
            "check_r2": str(check_r2_path),
        },
    }

    write_jsonl(parse_records_r2_path, parse_records_r2)
    write_jsonl(filelist_entries_r2_path, filelist_entries_r2)
    write_jsonl(reverse_index_r2_path, reverse_index_r2)
    summary_r2_path.write_text(json.dumps(summary, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")

    check_lines = [
        f"M22B_R2_MANIFEST_FILELIST_PARSE={final_status}",
        "",
        ...
        f"roa_uri_to_manifest_filelist_index_count = {len(reverse_index_r2)}",
        ...
        f"parse_records_r2_path = {parse_records_r2_path}",
        f"manifest_filelist_entries_r2_path = {filelist_entries_r2_path}",
        f"roa_uri_to_manifest_filelist_index_r2_path = {reverse_index_r2_path}",
        f"summary_r2_path = {summary_r2_path}",
    ]
    write_text(check_r2_path, "\n".join(check_lines) + "\n")
